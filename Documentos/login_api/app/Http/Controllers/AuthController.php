    ...
    public function login(Request $request)
    {
        ...
        if ($user && Hash::check($request->Clave, $user->Clave)) {
            if ($user->role == 'admin') {
                ...
                //return redirect('/admin')->with('message', 'Inicio de sesión exitoso.');
            }if ($user->role == 'superAdmin') {
                return response(["message"=>"Super Administrador Autenticado"],Response::HTTP_OK);
                //return redirect('/superAdmin')->with('message', 'Inicio de sesión exitoso.');
            } else {
                return response(["message"=>"Empleado Autenticado"],Response::HTTP_OK);
                //return redirect('/welcome')->with('message', 'Inicio de sesión exitoso.');
            }
